...
/* ---------------------------
   Product modal setup (multi-row)
   --------------------------- */
   (function setupProductsModule() {
    // --- defensive clone helper (clears old listeners) ---
    ...
    // Replace UI nodes so previously attached listeners don't conflict
    ['addProductBtn','emptyAddBtn','productForm','productModal','productModalBackdrop','closeModal','cancelModal','addProductRowBtn']
      .forEach(cloneReplaceById);
    ...
    const eHtml = (typeof escapeHtml === 'function') ? escapeHtml : _escapeHtml;
    ...
    function createProductRowNode({ id = '', name = '', cost = '', price = '', qty = '' } = {}, singleMode = false) {
      ...
      row.innerHTML = `
        <div class="col-span-12 sm:col-span-5">
          ...
          <input type="text" class="prod-name mt-1 w-full border rounded-xl px-3 py-2" placeholder="Product name" value="${eHtml(name)}" required />
        </div>
        <div class="col-span-6 sm:col-span-2">
          <label class="block text-sm text-gray-600 dark:text-gray-300">Cost</label>
          <input type="number" step="0.01" min="0" class="prod-cost mt-1 w-full border rounded-xl px-3 py-2" placeholder="0.00" value="${eHtml(cost)}" />
        </div>
        <div class="col-span-6 sm:col-span-2">
          <label class="block text-sm text-gray-600 dark:text-gray-300">Price *</label>
          <input type="number" step="0.01" min="0" class="prod-price mt-1 w-full border rounded-xl px-3 py-2" placeholder="0.00" value="${eHtml(price)}" required />
        </div>
        <div class="col-span-8 sm:col-span-2">
          <label class="block text-sm text-gray-600 dark:text-gray-300">Qty *</label>
          <input type="number" step="1" min="0" class="prod-qty mt-1 w-full border rounded-xl px-3 py-2" placeholder="0" value="${eHtml(qty)}" required />
        </div>
    ...
    function addProductRow(prefill = null) {
      const container = document.getElementById('productRowsModal');
    ...
    function openProductModal(isEdit = false) {
      ...
      const rowsEl = document.getElementById('productRowsModal');
    ...
    btnAdd?.addEventListener('click', e => { e.preventDefault(); window.editingProductId = null; openProductModal(false); });
    btnEmptyAdd?.addEventListener('click', e => { e.preventDefault(); window.editingProductId = null; openProductModal(false); });
    btnClose?.addEventListener('click', e => { e.preventDefault(); closeProductModal(); });
    btnCancel?.addEventListener('click', e => { e.preventDefault(); closeProductModal(); });
    btnBackdrop?.addEventListener('click', () => closeProductModal());
    btnAddRow?.addEventListener('click', e => { e.preventDefault(); addProductRow(); });
    ...
    form?.addEventListener('submit', (ev) => {
      ...
      const all = (typeof getAllProducts === 'function') ? getAllProducts() || [] : [];
      const rows = Array.from(document.getElementById('productRowsModal')?.children || []);
      ...
      if (window.editingProductId) {
        ...
        if (typeof saveAllProducts === 'function') saveAllProducts(all);
        closeProductModal();
        if (typeof renderProductList === 'function') renderProductList(searchInput?.value || '');
        window.dispatchEvent(new Event('dataUpdated'));
      ...
      if (typeof saveAllProducts === 'function') saveAllProducts(merged);
      closeProductModal();
      if (typeof renderProductList === 'function') renderProductList(searchInput?.value || '');
      window.dispatchEvent(new Event('dataUpdated'));
    ...
    // handle product actions (edit / buy / delete)
    ...
    // Attach action delegation to product rows/cards
    ...
    productRowsContainer?.addEventListener('click', e => {
      ...
      if (!btn) return;
      handleProductAction(btn.getAttribute('data-action'), btn.getAttribute('data-id'));
    });
    ...
    // Expose global alias so legacy code that expects a global still works
    window.handleProductAction = handleProductAction;
  
    // expose for debugging
    window._rb_products = {
      openProductModal, closeProductModal, addProductRow, handleProductAction
    };
  
  })(); // end setupProductsModule
...
function ensureSettingsBtnClass() {
  try {
    ...
    if (!btn) return;
    // add a stable class used elsewhere; keep it idempotent
    if (!btn.classList.contains('storeSettingsBtn')) btn.classList.add('storeSettingsBtn');
    // ensure it's focusable/clickable
    btn.style.pointerEvents = '';
    btn.setAttribute('aria-hidden', 'false');
  } catch (e) {
    // swallow errors to avoid aborting modal wiring
    console.warn('ensureSettingsBtnClass error', e);
  }
}
window.ensureSettingsBtnClass = ensureSettingsBtnClass;

/* -------------------------
   Ensure settings cog/button exists + helper
   ------------------------- */
   // --- ensureSettingsNavVisible must be defined before any early-return branch uses it ---
   function ensureSettingsNavVisible() {
    const nav = document.getElementById('settingsNav');
    if (!nav) return;
    // make nav visible and mobile-friendly
    ...
    nav.querySelectorAll('.settings-tab').forEach(tb => {
      ...
      // make sure pointer events are enabled (in case something disabled them)
      tb.style.pointerEvents = '';
      tb.setAttribute('aria-hidden', 'false');
    });
    // ensure the nav is focusable for keyboard users
    const firstTab = nav.querySelector('.settings-tab');
    if (firstTab && typeof firstTab.focus === 'function') firstTab.focus();
  }
  // also expose globally in case other modules rely on it
  window.ensureSettingsNavVisible = ensureSettingsNavVisible;
  ...
  <div class="relative w-full max-w-3xl bg-white dark:bg-indigo-950 rounded-lg shadow-2xl overflow-auto max-h-[92vh] ring-1 ring-indigo-50 dark:ring-indigo-800">
    <!-- Header -->
    <div class="flex items-center justify-between p-4 border-b border-indigo-100 dark:border-indigo-800">
      <div class="flex items-center gap-4">
        ...
        <div id="langToggle" class="inline-flex items-center bg-indigo-50 dark:bg-indigo-900/20 rounded-full p-1">
          <button id="langEnBtn" class="px-3 py-1 rounded-full text-xs font-semibold text-indigo-800 dark:text-indigo-100">EN</button>
          <button id="langSoBtn" class="px-3 py-1 rounded-full text-xs font-semibold text-indigo-800 dark:text-indigo-100">SO</button>
        </div>
      </div>
    ...
    <div class="md:flex md:gap-6">
      ...
      <!-- Content -->
      <div id="settingsContent" class="p-5 md:flex-1">
        <!-- Help Notice panel (modern animated card) -->
        <div class="settings-panel" data-panel="helpNotice" style="display:none">
          <div id="helpNoticeCard" class="help-notice-card flex items-start gap-4 p-5 rounded-2xl bg-gradient-to-r from-emerald-50 to-amber-50 dark:from-indigo-900 dark:to-indigo-800 border border-indigo-100 dark:border-indigo-800 shadow-md">
            <div class="text-3xl">💡</div>
            <div class="flex-1">
              <div class="flex items-start justify-between">
                <div>
                  <h4 id="helpNoticeTitle" class="font-bold text-lg md:text-xl text-indigo-900 dark:text-amber-100">Quick Tips</h4>
                  <div id="helpNoticeBody" class="mt-2 text-sm md:text-base leading-relaxed text-indigo-700 dark:text-indigo-200">
                    Short helpful context appears here. Use <strong>Create Invoice</strong> to begin and <strong>Add Product</strong> to populate stock.
                  </div>
                </div>
                <div class="ml-4">
                  <button id="dismissHelpNotice" class="text-xs px-3 py-1 rounded-md bg-amber-50 dark:bg-amber-700/20 text-amber-700 dark:text-amber-200">Dismiss</button>
                </div>
              </div>

              <div class="mt-4 flex flex-wrap gap-3">
                <button id="moreHelpBtn" class="px-4 py-2 bg-indigo-700 text-white rounded-md font-semibold shadow-sm hover:opacity-95"><i class="fa-solid fa-book-open mr-2"></i>Full Guide</button>
                <button id="showHelpPanelBtn" class="px-4 py-2 bg-amber-100 text-amber-800 rounded-md font-medium hover:shadow-sm">Open Help</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex-1">
        ...
        <div class="settings-panel" data-panel="notices" style="display:none">
          <div class="flex items-center justify-between mb-3">
            <h4 class="font-bold text-indigo-900 dark:text-amber-100">Notices</h4>
            <div class="flex items-center gap-2">
              <button id="translateProgrammaticNotices" class="px-3 py-1 text-xs rounded-md bg-amber-100 text-amber-800">Translate</button>
              <button id="clearNoticesBtn" class="px-3 py-1 text-xs rounded-md bg-red-600 text-white">Clear All</button>
            </div>
          </div>
          <div id="settingsNotices" class="space-y-3 max-h-56 overflow-auto p-1"></div>
        </div>

        <!-- Export -->
        <div class="settings-panel" data-panel="export" style="display:none">
          <h4 class="font-bold mb-3 text-indigo-900 dark:text-amber-100">Export</h4>
          <div class="flex gap-3 flex-wrap">
            <button id="exportInvoicesPdf" class="px-4 py-2 bg-indigo-700 text-white rounded-md"><i class="fa-solid fa-file-pdf mr-2"></i> PDF</button>
            <button id="exportInvoicesExcel" class="px-4 py-2 bg-emerald-600 text-white rounded-md"><i class="fa-solid fa-file-csv mr-2"></i> CSV</button>
          </div>
        </div>

        <!-- Drive -->
        <div class="settings-panel" data-panel="drive" style="display:none">
          <h4 class="font-bold mb-3 text-indigo-900 dark:text-amber-100">Google Drive Backup</h4>
          <div class="text-sm mb-3 text-indigo-700 dark:text-indigo-200">Requires Google OAuth &amp; test user acceptance. Backups snapshot localStorage as JSON.</div>
          <div class="space-y-3">
            ...
            <label class="flex items-center gap-2">
              ...
              <input id="optAutoBackupDays" type="number" min="1" value="7" class="ml-2 w-20 rounded-md border px-2 py-1"> days
            </label>
            <div class="text-xs text-indigo-500 dark:text-indigo-300">Auto backups run while the app is open. Last-run recorded in settings.</div>
            <div class="flex gap-3 mt-2 flex-wrap">
              <button id="driveBackupBtn" class="px-4 py-2 bg-indigo-700 text-white rounded-md"><i class="fa-brands fa-google-drive mr-2"></i> Backup</button>
              <button id="driveRefreshBtn" class="px-4 py-2 bg-amber-500 text-white rounded-md"><i class="fa-solid fa-refresh mr-2"></i> Refresh</button>
              <button id="driveRestoreLatestBtn" class="px-4 py-2 bg-red-600 text-white rounded-md"><i class="fa-solid fa-clock-rotate-left mr-2"></i> Restore</button>
            </div>
            <div id="driveStatus" class="mt-2 text-sm text-indigo-700 dark:text-indigo-200">Drive: not initialized</div>
